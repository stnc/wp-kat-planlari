...
function stnc_wp_floor_adminMenu_map_sabit()
{
    ...
    $stncForm_wp_stnc_map_floors ='stnc_map_floors';
$map = $wpdb->get_row($wpdb->prepare("SELECT bina.name AS bina,kat.name kat_adi,kat.tekno_id,kat.scheme,bina.id
 AS bina_id,kat.id AS katid  FROM wp_stnc_map_floors AS kat INNER JOIN wp_stnc_map_building  AS bina  ON  bina.id=%d AND kat.id = %d", $_GET['binaid'],$_GET['kat']));
        //    print_r( $thepost );
